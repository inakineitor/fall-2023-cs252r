...
class Continuation {
  ...
  pushRestoreEnv(env: Environment) {
    ...
    if (len > 0) if (this.stack[len - 1][0] === ContOp.RestoreEnv) return; // tail call
...
const G1 = c(
  '+',
  2,
  (x) => add(fst(x) as Numeric, snd(x) as Numeric),
  c(
    '-',
    2,
    (x) => subtract(fst(x) as Numeric, snd(x) as Numeric),
    c(
      '*',
      2,
      (x) => multiply(fst(x) as Numeric, snd(x) as Numeric),
      c(
        '<',
        2,
        (x) => compare(fst(x) as Numeric, snd(x) as Numeric) < 0,
        c(
          '=',
          2,
          (x) => compare(fst(x) as Numeric, snd(x) as Numeric) === 0,
  ...
  c(
    ...
    c(
      ...
      c(
        'cons',
        2,
        (x) => new Cell(fst(x), snd(x)),
        c(
          'eq?',
          2,
          (x) => Object.is(fst(x), snd(x)),
          c(
            'pair?',
            1,
            (x) => fst(x) instanceof Cell,
            c(
              'null?',
              1,
              (x) => fst(x) === null,
                      ...
                      c(
                        ...
                        c(
                          ...
                          c(
                            'symbol?',
                            1,
                            (x) => fst(x) instanceof Sym,
                            new Environment(
                              CallCCSym,
                              CallCCVal,
                              new Environment(ApplySym, ApplyVal, G1),
                            ),
                          ),
                        ),
                      ),
      ...
      for (;;) {
        if (exp instanceof Cell) {
          ...
          if (kar === QuoteSym) {
            ...
            // (lambda (v...) e...)
            exp = new Closure(kdr.car as Cell, kdr.cdr as Cell, env);
            break;
  ...
  if (fun instanceof Intrinsic) {
    if (fun.arity >= 0)
      if (arg === null ? fun.arity > 0 : arg.length !== fun.arity)
        throw Error('arity not matched: ' + fun + ' and ' + stringify(arg));
...
if (typeof Deno !== 'undefined') {
  ...
  readLine = async function (): Promise<string | null> {
    ...
    if (n === null) return null; // End-Of-File
